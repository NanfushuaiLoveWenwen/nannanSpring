...
public abstract class AbstractBeanFactory extends FactoryBeanRegistrySupport implements ConfigurableBeanFactory, BeanDefinitionRegistry {
    ...
    public Object getBean(String beanName) throws BeansException {
        ...
        if (singletonBean == null) {
            ...
            //3.1 初始化factoryBean 对所有实现了beanFactoryAware的bean 传递factory信息
            if(singletonBean instanceof BeanFactoryAware){
                ((BeanFactoryAware) singletonBean).setBeanFactory(this);
            }

            //4 bean post processor 在创建bean 之后执行
            //4.1 post processor before initialization 目前支持:
            //a. autowired注解
            //b. aopProxyFactory 替换原来的bean
            singletonBean = applyBeanPostProcessorsBeforeInitialization(singletonBean, beanName);
            ...
            this.registerBean(beanName, singletonBean);
        }

        //支持 factory bean
        if(singletonBean instanceof FactoryBean){
            System.out.println("factory bean -------------- " + beanName + "----------------"+singletonBean);
            return this.getObjectForBeanInstance(singletonBean, beanName);
        }else{
            System.out.println("normal bean -------------- " + beanName + "----------------"+singletonBean);
    ...
    protected Object getObjectForBeanInstance(Object beanInstance, String beanName) {
        ...
        return super.getObjectFromFactoryBean(factoryBean, beanName);
    }
}
